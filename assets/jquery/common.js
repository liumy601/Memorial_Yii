...
CAMPUS.subpanelUtils={};
  
CAMPUS.ajaxStatusClass={};
...
function validate_form(formname,startsWith){
  requiredTxt=CAMPUS.language.get('app_strings','ERR_MISSING_REQUIRED_FIELDS');
  invalidTxt=CAMPUS.language.get('app_strings','ERR_INVALID_VALUE');
        ...
        if(typeof validate[formname][i][jstypeIndex]!='undefined'){
          switch(validate[formname][i][jstypeIndex]){
            case'range':
              ...
              isError=true;
              var lbl_validate_range=CAMPUS.language.get('app_strings','LBL_VALIDATE_RANGE');
            ...
            case'isbefore':
              ...
              if(trim(compareTo.value)!=''||(validate[formname][i][allowblank]!='true')){
                ...
                if(trim(date1).length!=0&&!isBefore(date1,date2)){
                  isError=true;
                  add_error_style(formname,validate[formname][i][nameIndex],validate[formname][i][msgIndex]+"("+date1+") "+CAMPUS.language.get('app_strings','MSG_IS_NOT_BEFORE')+' '+date2);
                }
              }
            }
          break;
...
if(formWithPrecision){
  if(!isValidPrecision(formWithPrecision.float.value,formWithPrecision.precision.value)){
    isError=true;
    add_error_style(formname,'default',CAMPUS.language.get('app_strings','ERR_COMPATIBLE_PRECISION_VALUE'));
  }else if(!isError){
    isError=false;
  }
}
...
function sendAndRetrieve(theForm,theDiv,loadingStr){
  ...
  if(typeof loadingStr=='undefined')CAMPUS.language.get('app_strings','LBL_LOADING');
...
if(typeof savingStr=='undefined')CAMPUS.language.get('app_strings','LBL_LOADING');
...
sugarListView.prototype.confirm_action=function(del){
  if(del==1){
    return confirm(CAMPUS.language.get('app_strings','NTC_DELETE_CONFIRMATION_NUM')+sugarListView.get_num_selected()+CAMPUS.language.get('app_strings','NTC_DELETE_SELECTED_RECORDS'));
  }
  else{
    return confirm(CAMPUS.language.get('app_strings','NTC_UPDATE_CONFIRMATION_NUM')+sugarListView.get_num_selected()+CAMPUS.language.get('app_strings','NTC_DELETE_SELECTED_RECORDS'));
  }
}
...
sugarListView.prototype.send_form=function(select,currentModule,action,no_record_txt,action_module,return_info){
  if(document.MassUpdate.select_entire_list.value==1){
    var href=action+'&module='+currentModule;
    if(return_info)
      href+=return_info;
    var newForm=document.createElement('form');
    newForm.method='post';
...
sugarListView.prototype.check_item=function(cb,form){
  if(cb.checked)sugarListView.update_count(1,true);else sugarListView.update_count(-1,true);
}
...
CAMPUS.ajaxStatusClass=function(){};
  
CAMPUS.ajaxStatusClass.prototype.statusDiv=null;
CAMPUS.ajaxStatusClass.prototype.oldOnScroll=null;
CAMPUS.ajaxStatusClass.prototype.shown=false;
CAMPUS.ajaxStatusClass.prototype.positionStatus=function(){
...
CAMPUS.ajaxStatusClass.prototype.createStatus=function(text){
...
CAMPUS.ajaxStatusClass.prototype.showStatus=function(text){
  if(!this.statusDiv){
...
CAMPUS.ajaxStatusClass.prototype.hideStatus=function(text){
  if(!this.shown)return;
  this.shown=false;
  if(this.oldOnScroll)window.onscroll=this.oldOnScroll;else window.onscroll='';
  this.statusDiv.style.display='none';
}
CAMPUS.ajaxStatusClass.prototype.flashStatus=function(text,time){
  this.showStatus(text);
  window.setTimeout('ajaxStatus.hideStatus();',time);
}
var ajaxStatus=new CAMPUS.ajaxStatusClass();
CAMPUS.unifiedSearchAdvanced=function(){
  var usa_div;
  var usa_img;
  var usa_open;
  var usa_content;
  ...
  return{
    init:function(){
      CAMPUS.unifiedSearchAdvanced.usa_div=document.getElementById('unified_search_advanced_div');
      CAMPUS.unifiedSearchAdvanced.usa_img=document.getElementById('unified_search_advanced_img');
      if(!CAMPUS.unifiedSearchAdvanced.usa_div||!CAMPUS.unifiedSearchAdvanced.usa_img)return;
      CAMPUS.unifiedSearchAdvanced.anim_open=new YAHOO.util.Anim('unified_search_advanced_div',{
        height:{
          to:300
        }
      });
    CAMPUS.unifiedSearchAdvanced.anim_open.duration=0.75;
    CAMPUS.unifiedSearchAdvanced.anim_close=new YAHOO.util.Anim('unified_search_advanced_div',{
      height:{
        to:1
      }
    });
  CAMPUS.unifiedSearchAdvanced.anim_close.duration=0.75;
  CAMPUS.unifiedSearchAdvanced.anim_close.onComplete.subscribe(function(){
    CAMPUS.unifiedSearchAdvanced.usa_div.style.display='none'
    });
  CAMPUS.unifiedSearchAdvanced.usa_img._x=YAHOO.util.Dom.getX(CAMPUS.unifiedSearchAdvanced.usa_img);
  CAMPUS.unifiedSearchAdvanced.usa_img._y=YAHOO.util.Dom.getY(CAMPUS.unifiedSearchAdvanced.usa_img);
  CAMPUS.unifiedSearchAdvanced.usa_open=false;
  CAMPUS.unifiedSearchAdvanced.usa_content=null;
  YAHOO.util.Event.addListener('unified_search_advanced_img','click',CAMPUS.unifiedSearchAdvanced.get_content);
},
get_content:function(e){
  if(CAMPUS.unifiedSearchAdvanced.usa_content==null){
    ajaxStatus.showStatus(CAMPUS.language.get('app_strings','LBL_LOADING'));
    var cObj=YAHOO.util.Connect.asyncRequest('GET','index.php?to_pdf=1&module=Home&action=UnifiedSearch&usa_form=true',{
      success:CAMPUS.unifiedSearchAdvanced.animate,
      failure:CAMPUS.unifiedSearchAdvanced.animate
      },null);
  }
  else CAMPUS.unifiedSearchAdvanced.animate();
},
animate:function(data){
  ajaxStatus.hideStatus();
  if(data){
    CAMPUS.unifiedSearchAdvanced.usa_content=data.responseText;
    CAMPUS.unifiedSearchAdvanced.usa_div.innerHTML=CAMPUS.unifiedSearchAdvanced.usa_content;
  }
  if(CAMPUS.unifiedSearchAdvanced.usa_open){
    document.UnifiedSearch.advanced.value='false';
    CAMPUS.unifiedSearchAdvanced.anim_close.animate();
  }
  else{
    document.UnifiedSearch.advanced.value='true';
    CAMPUS.unifiedSearchAdvanced.usa_div.style.display='';
    YAHOO.util.Dom.setX(CAMPUS.unifiedSearchAdvanced.usa_div,CAMPUS.unifiedSearchAdvanced.usa_img._x-90);
    YAHOO.util.Dom.setY(CAMPUS.unifiedSearchAdvanced.usa_div,CAMPUS.unifiedSearchAdvanced.usa_img._y+15);
    CAMPUS.unifiedSearchAdvanced.anim_open.animate();
  }
  CAMPUS.unifiedSearchAdvanced.usa_open=!CAMPUS.unifiedSearchAdvanced.usa_open;
  return false;
},
...
if(typeof YAHOO!='undefined')YAHOO.util.Event.addListener(window,'load',CAMPUS.unifiedSearchAdvanced.init);
CAMPUS.ui={
  ...
  alert(CAMPUS.language.get("app_strings","ERR_NO_HEADER_ID"));
}
}
};

CAMPUS.util=function(){
  ...
  return{
    evalScript:function(text){
      if(isSafari){
        var waitUntilLoaded=function(){
          CAMPUS.evalScript_waitCount--;
          if(CAMPUS.evalScript_waitCount==0){
            var headElem=document.getElementsByTagName('head')[0];
            for(var i in CAMPUS.evalScript_evalElem){
              var tmpElem=document.createElement('script');
              tmpElem.type='text/javascript';
              tmpElem.text=CAMPUS.evalScript_evalElem[i];
    ...
    var headElem=document.getElementsByTagName('head')[0];
    var tmpElem=null;
    CAMPUS.evalScript_waitCount=0;
    CAMPUS.evalScript_evalElem=new Array();
    for(var i in results){
      ...
      if(results[i].src!=null&&results[i].src!=''){
        ...
        CAMPUS.evalScript_evalElem[CAMPUS.evalScript_evalElem.length]=results[i].text;
        continue;
      }
...
setMaxLength:function(){
  ...
  for(var i=0;i<x.length;i++){
    if(x[i].getAttribute('maxlength')){
      x[i].onkeyup=x[i].onchange=CAMPUS.util.checkMaxLength;
      x[i].onkeyup();
    }
  }
  },
getAdditionalDetails:function(bean,id,spanId){
  go=function(){
    oReturn=function(body,caption,width,theme){
      return overlib(body,CAPTION,caption,STICKY,MOUSEOFF,1000,WIDTH,width,CLOSETEXT,('<img border=0 style="margin-left:2px; margin-right: 2px;" src=themes/'+theme+'/images/close.gif>'),CLOSETITLE,CAMPUS.language.get('app_strings','LBL_ADDITIONAL_DETAILS_CLOSE_TITLE'),CLOSECLICK,FGCLASS,'olFgClass',CGCLASS,'olCgClass',BGCLASS,'olBgClass',TEXTFONTCLASS,'olFontClass',CAPTIONFONTCLASS,'olCapFontClass',CLOSEFONTCLASS,'olCloseFontClass',REF,spanId,REFC,'LL',REFX,13);
    }
    success=function(data){
      eval(data.responseText);
      CAMPUS.util.additionalDetailsCache[spanId]=new Array();
      CAMPUS.util.additionalDetailsCache[spanId]['body']=result['body'];
      CAMPUS.util.additionalDetailsCache[spanId]['caption']=result['caption'];
      CAMPUS.util.additionalDetailsCache[spanId]['width']=result['width'];
      CAMPUS.util.additionalDetailsCache[spanId]['theme']=result['theme'];
      ajaxStatus.hideStatus();
      return oReturn(CAMPUS.util.additionalDetailsCache[spanId]['body'],CAMPUS.util.additionalDetailsCache[spanId]['caption'],CAMPUS.util.additionalDetailsCache[spanId]['width'],CAMPUS.util.additionalDetailsCache[spanId]['theme']);
    }
    if(typeof CAMPUS.util.additionalDetailsCache[spanId]!='undefined')
      return oReturn(CAMPUS.util.additionalDetailsCache[spanId]['body'],CAMPUS.util.additionalDetailsCache[spanId]['caption'],CAMPUS.util.additionalDetailsCache[spanId]['width'],CAMPUS.util.additionalDetailsCache[spanId]['theme']);
    if(typeof CAMPUS.util.additionalDetailsCalls[spanId]!='undefined')
      return;
    ajaxStatus.showStatus(CAMPUS.language.get('app_strings','LBL_LOADING'));
    url='index.php?to_pdf=1&module=Home&action=AdditionalDetailsRetrieve&bean='+bean+'&id='+id;
    CAMPUS.util.additionalDetailsCalls[spanId]=YAHOO.util.Connect.asyncRequest('GET',url,{
      success:success,
      failure:success
    });
    return false;
  }
  CAMPUS.util.additionalDetailsRpcCall=window.setTimeout('go()',250);
},
clearAdditionalDetailsCall:function(){
  if(typeof CAMPUS.util.additionalDetailsRpcCall=='number')window.clearTimeout(CAMPUS.util.additionalDetailsRpcCall);
}
};

}();
CAMPUS.util.additionalDetailsCache=new Array();
CAMPUS.util.additionalDetailsCalls=new Array();
if(typeof YAHOO!='undefined')YAHOO.util.Event.addListener(window,'load',CAMPUS.util.setMaxLength);
CAMPUS.savedViews=function(){
  var selectedOrderBy;
...
saved_search_action:function(action,delete_lang){
  ...
  if(action=='save'){
    if(document.search_form.saved_search_name.value.replace(/^\s*|\s*$/g,'')==''){
      alert(CAMPUS.language.get('app_strings','LBL_SAVED_SEARCH_ERROR'));
      return;
    }
  }
...
handleForm:function(){
  CAMPUS.tabChooser.movementCallback=function(left_side,right_side){
    ...
    for(i in left_side.childNodes){
      if(typeof left_side.childNodes[i].nodeName!='undefined'&&left_side.childNodes[i].nodeName.toLowerCase()=='option'&&typeof CAMPUS.savedViews.columnsMeta[left_side.childNodes[i].value]!='undefined'&&typeof CAMPUS.savedViews.columnsMeta[left_side.childNodes[i].value]['sortable']=='undefined'&&CAMPUS.savedViews.columnsMeta[left_side.childNodes[i].value]['sortable']!=false){
        nodeCount++;
        ...
        if(optionNode.value==CAMPUS.savedViews.selectedOrderBy)
...
CAMPUS.tabChooser.movementCallback(document.getElementById('display_tabs_td').getElementsByTagName('select')[0]);
if(document.search_form.orderBy)
  document.search_form.orderBy.options.value=CAMPUS.savedViews.selectedOrderBy;
if(CAMPUS.savedViews.selectedSortOrder=='DESC')document.getElementById('sort_order_desc_radio').checked=true;else document.getElementById('sort_order_asc_radio').checked=true;
}
};

}();
CAMPUS.searchForm=function(){
  var url;
  return{
      ...
      if(document.search_form.elements[num]){
        ...
        if(typeof el.type!='undefined'&&typeof el.name!='undefined'&&el.name.match(pattern)){
          ...
          if(typeof document.search_form[advanced_input_name]!='undefined')
            CAMPUS.searchForm.copyElement(advanced_input_name,el);
        }
      }
    }
    }
if(document.getElementById(module+theView+'SearchForm').innerHTML==''){
  ...
  var success=function(data){
    ...
    CAMPUS.util.evalScript(data.responseText);
    if(theView=='saved_views'){
      if(typeof columnsMeta!='undefined')CAMPUS.savedViews.columnsMeta=columnsMeta;
      if(typeof selectedOrderBy!='undefined')CAMPUS.savedViews.selectedOrderBy=selectedOrderBy;
      if(typeof selectedSortOrder!='undefined')CAMPUS.savedViews.selectedSortOrder=selectedSortOrder;
    }
    handleDisplay();
    enableQS(true);
    ajaxStatus.hideStatus();
...
if(theView=='saved_views')
  url+='&displayColumns='+CAMPUS.savedViews.displayColumns+'&hideTabs='+CAMPUS.savedViews.hideTabs+'&orderBy='+CAMPUS.savedViews.selectedOrderBy+'&sortOrder='+CAMPUS.savedViews.selectedSortOrder;
var cObj=YAHOO.util.Connect.asyncRequest('GET',url,{
  success:success,
  failure:success
});
...
CAMPUS.tabChooser=function(){
  ...
  return{
    ...
    freezeOptions:function(left_name,right_name,target){
      if(!CAMPUS.tabChooser.frozenOptions){
        CAMPUS.tabChooser.frozenOptions=[];
      }
      if(!CAMPUS.tabChooser.frozenOptions[left_name]){
        CAMPUS.tabChooser.frozenOptions[left_name]=[];
      }
      if(!CAMPUS.tabChooser.frozenOptions[left_name][right_name]){
        CAMPUS.tabChooser.frozenOptions[left_name][right_name]=[];
      }
      if(typeof target=='array'){
        for(var i in target){
          CAMPUS.tabChooser.frozenOptions[left_name][right_name][target[i]]=true;
        }
        }else{
      CAMPUS.tabChooser.frozenOptions[left_name][right_name][target]=true;
    }
  },
...
left_to_right:function(left_name,right_name,left_size,right_size){
  ...
  var frozen_options=CAMPUS.tabChooser.frozenOptions;
...
right_to_left:function(left_name,right_name,left_size,right_size,max_left){
  ...
  var notselected_left=new Array();
  var frozen_options=CAMPUS.tabChooser.frozenOptions;
  frozen_options=CAMPUS.tabChooser.frozenOptions&&CAMPUS.tabChooser.frozenOptions[right_name]&&CAMPUS.tabChooser.frozenOptions[right_name][left_name]?CAMPUS.tabChooser.frozenOptions[right_name][left_name]:[];
  for(i=0;i<hidden_columns_ref.options.length;i++)
...
CAMPUS.language=function(){
  return{
    languages:new Array(),
    setLanguage:function(module,data){
      if(!CAMPUS.language.languages){}
      CAMPUS.language.languages[module]=data;
    },
    get:function(module,str){
      if(typeof CAMPUS.language.languages[module]=='undefined'||typeof CAMPUS.language.languages[module][str]=='undefined')
        return'undefined';
      return CAMPUS.language.languages[module][str];
    }
  };

}();
CAMPUS.contextMenu=function(){
  return{
    objects:new Object(),
    objectTypes:new Object(),
    registerObject:function(objectType,id,metaData){
      CAMPUS.contextMenu.objects[id]=new Object();
      CAMPUS.contextMenu.objects[id]={
  ...
  registerObjectType:function(name,menuItems){
    CAMPUS.contextMenu.objectTypes[name]=new Object();
    CAMPUS.contextMenu.objectTypes[name]={
      'menuItems':menuItems,
      'objects':new Array()
      };
...
onContextMenuItemClick:function(p_sType,p_aArguments,p_oItem){
  var oLI=CAMPUS.contextMenu.getListItemFromEventTarget(this.parent.contextEventTarget);
  id=this.parent.contextEventTarget.parentNode.id;
  funct=eval(CAMPUS.contextMenu.objectTypes[CAMPUS.contextMenu.objects[id]['objectType']]['menuItems'][this.index]['action']);
  funct(this.parent.contextEventTarget,CAMPUS.contextMenu.objects[id]['metaData']);
},
init:function(){
  for(var i in CAMPUS.contextMenu.objects){
    if(typeof CAMPUS.contextMenu.objectTypes[CAMPUS.contextMenu.objects[i]['objectType']]['objects']=='undefined')
      CAMPUS.contextMenu.objectTypes[CAMPUS.contextMenu.objects[i]['objectType']]['objects']=new Array();
    CAMPUS.contextMenu.objectTypes[CAMPUS.contextMenu.objects[i]['objectType']]['objects'].push(document.getElementById(i));
  }
  for(var i in CAMPUS.contextMenu.objectTypes){
    var oContextMenu=new YAHOO.widget.ContextMenu(i,{
      'trigger':CAMPUS.contextMenu.objectTypes[i]['objects']
      });
    var aMainMenuItems=CAMPUS.contextMenu.objectTypes[i]['menuItems'];
    ...
    for(var j=0;j<nMainMenuItems;j++){
      ...
      oMenuItem.clickEvent.subscribe(CAMPUS.contextMenu.onContextMenuItemClick,oMenuItem,true);
      oContextMenu.addItem(oMenuItem);
    }
    oContextMenu.moveEvent.subscribe(CAMPUS.contextMenu.onContextMenuMove,oContextMenu,true);
    oContextMenu.keyDownEvent.subscribe(CAMPUS.contextMenu.onContextMenuItemClick,oContextMenu,true);
...
CAMPUS.contextMenu.actions=function(){
  return{
...
if(label_data_str!=label_str&&current_label_data_str!=label_str){
  if(confirm(CAMPUS.language.get('app_strings','NTC_OVERWRITE_ADDRESS_PHONE_CONFIRM')+'\n\n'+label_data_str))
 ...
 * IN WHICH THE COPYRIGHT IS OWNED BY CAMPUSCRM, CAMPUSCRM DISCLAIMS THE WARRANTY
...
function enableQS(noReload){
  Ext.onReady(function(){
    ...
    var search=new Ext.form.ComboBox({
      ...
      typeAhead:true,
      loadingText:CAMPUS.language.get('app_strings','LBL_SEARCHING'),
...
function clearFields(sqs_object,filter){
  for(var key in sqs_object.field_list){
    if(isInteger(key)&&sqs_object.populate_list[key].match(filter))
